...
9. The paired-reference typography and conjoined-bubble corrections have passed the targeted real-page regressions. The next useful step is a fresh full 70-page personal-reading build with the accepted revision, followed by user review; do not add more layout rules until that reading exposes a concrete failure.
10. Keep semantic/OCR residuals separate from this layout correction: page 20's mixed-Latin term, page 60's wording failure, and intentionally preserved narration/SFX need later targeted work rather than being hidden by typography changes.
