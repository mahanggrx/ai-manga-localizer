...
12. A page-by-page review of the 70-page paired-reference build found that vertical lettering and artwork preservation were broadly usable, while only about half the pages were immediately readable under a relaxed personal-MVP standard. The dominant failures were story-bearing outside text left in Japanese and a smaller set of polarity, subject/object, action-direction, relationship, and time-relation mistranslations. The next iteration therefore broadens only sentence-like outside-text coverage using non-destructive complex-background rendering and adds evidence-driven semantic fallback checks; it does not reopen broad layout work or translate every artistic SFX.
13. The small real regression on pages 11, 22, 29, 42, 48, 51, 54, and 65 confirmed improvements for flaccid state, release permission, absorbed-vs-dream, and preparation time. Visual re-reading also showed that page 11 was not a relationship mistranslation, so no speculative relationship rule was retained. Complex-background story text improved on page 48 but dense multi-column text overlapped on page 65; validate the minimal grouping/font correction on page 65 before accepting this iteration or rerunning the full book.
14. The page-65 grouping/font regression still produced overlapping Chinese and incomplete source-text removal. For the personal MVP, fail conservatively when a page has more than three eligible outside-text regions: keep those regions unchanged while continuing normal bubble translation. Do not build a general multi-column outside-lettering engine before user reading evidence shows that untranslated outside narration is more harmful than damaged layout.
15. The final page-65 regression confirmed that the conservative density gate preserves the original page without overlap, black blocks, or source/target overprinting. It does not match the professional edition's translated multi-column narration, so record dense outside narration as a known coverage gap rather than reopening layout work before the next user reading pass.
16. A fresh 70-page build completed without page failures. Targeted visual comparison confirmed the state, release-permission, absorbed-vs-dream, preparation-time, and isolated outside-text improvements. The remaining page-54 leg-hold omission was fixed with one narrow deterministic source/target correction after both local models missed it; the corrected page passed a real single-page render and was assembled with the other 69 verified pages into a final personal-reading CBZ. Further work should come from the user's next reading pass, not another speculative broad audit.
